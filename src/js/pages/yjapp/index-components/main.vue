<template>
		<waterfall :show-scrollbar="false">
			...
			<cell style="background-color: #fff;" v-for="(obj , i ) in centerList" :key="i">
				...
				<div class="center">
					<div v-for="(item , index) in obj.pages" :key="index" v-if="item.isOther"  class="center-item" >
	...
	export default {
	    ...
	    computed :  {
			...
			centerList () {
				...
				if(userInfo.RoleId == 1 || userInfo.RoleId == 11 || userInfo.RoleId == 23){
					for(var i in distribution){
						var pages = distribution[i].pages;
						for(var j in pages){
							pages[j].isOther = true;
						}
					}
				}
				return distribution
